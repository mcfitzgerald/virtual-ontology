...
import pickle
import json
import dill  # More powerful than pickle for complex objects
...
class StateManager:
    ...
    def restore_simulation(self,
                          checkpoint_id: str,
                          primitive_classes: Dict[str, Type]) -> Tuple[simpy.Environment, Dict[str, Any]]:
...
class SimulationRunner:
    ...
    def run_chunked(self,
                   ...
                   checkpoint_interval: Optional[float] = None) -> Dict[str, Any]:
        ...
        while self.env.now < total_time:
            ...
            # Auto checkpoint
            if checkpoint_interval_minutes and \
               (self.env.now - last_checkpoint_time) >= checkpoint_interval_minutes:
                
                metadata = {
                    "chunk": chunks_completed + 1,
                    "memory_mb": memory_mb,
                    "events": total_events
                }
                ...
                last_checkpoint_time = self.env.now
        ...
        if self.monitor:
            results["monitor_metrics"] = self.monitor.get_metrics()
    ...
    def resume_from_checkpoint(self,
                              checkpoint_id: str,
                              primitive_classes: Dict[str, Type]) -> None:
        ...
        """
        self.env, self.primitives = self.state_manager.restore_simulation(
            checkpoint_id,
            primitive_classes
        )
