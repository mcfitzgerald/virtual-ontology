"""Equipment primitive V2 with internal queues only.
...
class EquipmentPrimitiveV2(BasePrimitive):
    ...
    def __init__(
        ...
        self.upstream_equipment: Optional['EquipmentPrimitiveV2'] = None
        self.downstream_equipment: Optional['EquipmentPrimitiveV2'] = None
    ...
    def _process_unit(self) -> Generator:
        ...
        product_performance = self.config.get_property(
            f"performance_by_product.{self.current_product}", 
            1.0
        )
    ...
    def _failure_process(self) -> Generator:
        ...
        while True:
            ...
            if self.state == EquipmentState.RUNNING and self.process:
                ...
                logger.info(f"{self.config.id}: {failure_type.value} occurred at {self.env.now:.1f}, repair: {duration:.1f} min")
    ...
    def _get_next_failure(self) -> Tuple[FailureType, float]:
        ...
        """
        # Use instance variables that are set from control manager
        micro_stop_prob = self.micro_stop_probability  # Use consistent naming
        minor_failure_prob = self.config.get_property("minor_failure_probability", 0.05)
        major_failure_prob = self.config.get_property("major_failure_probability", 0.02)
        ...
        if micro_stop_prob > 0:
            # micro_stop_prob is probability per 5 minutes
            # Convert to rate: if prob=0.3 per 5 min, rate = -ln(1-0.3)/5 = 0.0714/min
            micro_stop_rate = -math.log(1 - micro_stop_prob) / 5.0 * equipment_wear
        ...
        if minor_failure_prob > 0:
            ...
            minor_shape = 2.0  # Wear-out characteristic
            minor_scale = 480.0 / (minor_failure_prob * equipment_wear)  # Adjust scale by probability
        ...
        if major_failure_prob > 0:
            ...
            major_mean = np.log(2880.0 / (major_failure_prob * equipment_wear))
            major_sigma = 1.0
    ...
    def _get_repair_duration(self, failure_type: FailureType) -> float:
        ...
        if failure_type == FailureType.MICRO_STOP:
            # Micro-stops have specific causes with different durations
            cause_weights = {
                'jam_at_handoff': 0.40,      # 40% - Quick to clear
                'sensor_trip': 0.25,          # 25% - Reset needed
                'minor_adjustment': 0.15,     # 15% - Small tweaks
                'material_issue': 0.20        # 20% - Material removal
            }
            
            # Select cause based on weights
            cause = random.choices(
                list(cause_weights.keys()),
                weights=list(cause_weights.values())
            )[0]
            
            # Base durations by cause
            base_durations = {
                'jam_at_handoff': 1.0,       # 1 minute base
                'sensor_trip': 0.5,           # 30 seconds base
                'minor_adjustment': 2.0,      # 2 minutes base
                'material_issue': 1.5         # 1.5 minutes base
            }
            
            base_duration = base_durations[cause]
            # Add variability and apply operator skill factor
            duration = base_duration * np.random.lognormal(0, 0.3) / operator_skill
            ...
            scale = 7.5 / maintenance_effectiveness  # Scale affected by maintenance quality
            ...
            shape = 1.5  # Between exponential and normal
            # Scale based on MTTR and maintenance effectiveness
            scale = self.mttr / maintenance_effectiveness
    ...
    def get_kpis(self) -> Dict[str, float]:
        ...
        failure_time = self.state_durations[EquipmentState.STOPPED_FAILURE]
