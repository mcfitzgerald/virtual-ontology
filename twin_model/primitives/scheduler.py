...
if TYPE_CHECKING:
    from .source import SourcePrimitive
...
class SchedulerPrimitive(BasePrimitive):
    ...
    def __init__(
        ...
        self.strategy = SequencingStrategy(config.get_property("sequencing_strategy", "changeover_optimized"))
        ...
        # Source connections
        self.line_sources: Dict[str, "SourcePrimitive"] = {}  # LINE1 -> Source mapping
        ...
        self.shifts_config = config.get_property(
            "shifts",
            {
        ...
        for i, prod1 in enumerate(product_list):
            for j, prod2 in enumerate(product_list):
                if i == j:
                    ...
                    if p1.family in allergen_families and p2.family not in allergen_families:
                        # Going from allergen to non-allergen: deep clean
                        allergen_adjustment = 25.0
                    elif p1.family not in allergen_families and p2.family in allergen_families:
                    ...
                    total_time = base_time + complexity_adjustment + allergen_adjustment + volume_adjustment
    ...
    def register_source(self, line_id: str, source: "SourcePrimitive") -> None:
    ...
    def _optimize_changeovers(self, orders: List[ProductionOrder]) -> List[ProductionOrder]:
        ...
        while remaining:
            ...
            for order in remaining:
                time = self.changeover_matrix.get_changeover_time(last_product, order.product.product_id)
    ...
    def _create_campaigns(self, orders: List[ProductionOrder]) -> List[ProductionOrder]:
        ...
        sorted_groups = sorted(product_groups.items(), key=lambda x: sum(o.quantity for o in x[1]), reverse=True)
    ...
    def _update_shift(self) -> None:
        ...
        if current_shift_index != self.current_shift:
            ...
            self.emit_observable(
                "shift_change",
                {"from_shift": self.current_shift, "to_shift": current_shift_index, "time": self.env.now},
            )

            self.current_shift = current_shift_index
            self.shift_start_time = self.env.now

    def complete_order(self, order: ProductionOrder, completed_quantity: int, scrap_quantity: int = 0) -> None:
    ...
    def request_changeover(self, from_product: str, to_product: str) -> float:
        ...
        self.emit_observable(
            "changeover_started",
            {"from_product": from_product, "to_product": to_product, "duration": changeover_time, "time": self.env.now},
        )
