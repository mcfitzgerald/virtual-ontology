...
from typing import Dict, Any, Optional, List, Callable, Deque, Set, Tuple, DefaultDict, Generator
...
try:
    ...
    _twin_config = {
        ...
        "monitoring": {"memory_threshold_mb": 500.0, "event_rate_threshold": 100.0, "check_interval": 60.0},
    }
...
class ObservableBuffer:
    ...
    def __init__(
        self, buffer_size: int = 1000, flush_callback: Optional[Callable[[List[Dict[str, Any]]], None]] = None
    ) -> None:
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        return {
            ...
            "utilization_percent": int((len(self.buffer) / self.buffer_size * 100)) if self.buffer_size > 0 else 0,
        }
...
class EventBatcher:
    ...
    def add_event(self, event: Dict[str, Any], timestamp: float) -> Optional[List[Dict[str, Any]]]:
    ...
    def _flush_old_windows(self, new_window: int) -> List[List[Dict[str, Any]]]:
        ...
        keys_to_flush = [key for key in self.pending_batches if key[1] < new_window]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "batches_by_type": {event_type: len(batch) for (event_type, _), batch in self.pending_batches.items()},
        }
...
class IncrementalAggregator:
    ...
    def add_value(self, metric_name: str, value: float, timestamp: float) -> Optional[Dict[str, Any]]:
    ...
    def _complete_window(self, timestamp: float) -> Dict[str, Any]:
        ...
        for metric_name, aggregator in self.metrics.items():
            ...
            if stats["count"] > 0:  # Only include metrics with data
                summary["metrics"][metric_name] = stats  # type: ignore[index]
    ...
    def get_current_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {name: agg.get_stats() for name, agg in self.metrics.items()},
        }
...
class ProgressCallback(ABC):
    ...
    @abstractmethod
    def __call__(
        self, current_time: float, total_time: float, events_processed: int, memory_usage_mb: float, **kwargs: Any
    ) -> None:
...
class SimulationMonitor:
    ...
    def __init__(
        ...
        event_rate_threshold: Optional[float] = None,
    ...
    def _monitor_process(self) -> Generator[simpy.Event, Any, Any]:
        ...
        while True:
            ...
            self.metrics["memory_samples"].append({"time": self.env.now, "memory_mb": memory_mb})
            ...
            if time_delta > 0:
                ...
                self.metrics["event_rate_samples"].append({"time": self.env.now, "rate": event_rate})
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_current_mb": self.metrics["memory_samples"][-1]["memory_mb"]
            if self.metrics["memory_samples"]
            else 0,
            "memory_max_mb": max(s["memory_mb"] for s in self.metrics["memory_samples"])
            if self.metrics["memory_samples"]
            else 0,
...
class BasePrimitive(ABC):
    ...
    def __init__(
        ...
        self.observable_buffer = ObservableBuffer(buffer_size=self.sampling_config.buffer_size)
        ...
        self.metric_aggregator = IncrementalAggregator(window_size=self.sampling_config.aggregation_interval)
    ...
    def emit_observable(
        ...
        observable = {
            "timestamp": self.env.now,
            "datetime": datetime.fromtimestamp(self.env.now * 60),  # Convert minutes to datetime
            "primitive_id": self.config.id,
            "primitive_type": self.config.type,
            "event_type": event_type,
            "severity": severity,
    ...
    def shutdown(self) -> None:
        ...
        self.emit_observable(event_type="primitive_shutdown", details={"final_state": self.get_state()})

    def emit_metric(self, metric_name: str, value: float) -> Optional[Dict[str, Any]]:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_saved_percent": (1 - self.events_sampled / self.events_emitted) * 100
