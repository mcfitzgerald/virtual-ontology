...
class SinkPrimitive(BasePrimitive):
    ...
    def __init__(
        ...
        # Throughput tracking
        self.throughput_window = self.get_system_config("kpi.throughput_window", 60.0)  # Default 1 hour
    ...
    def register_order(self, order_id: str, target_quantity: int) -> None:
        ...
        if self.order_tracking_enabled:
            self.active_orders[order_id] = OrderTracking(
                order_id=order_id, target_quantity=target_quantity, start_time=self.env.now
            )
            ...
            if isinstance(product, ProductionUnit):
                ...
                if product.quality < self.quality_threshold:
                    ...
                    self.emit_observable(
                        "product_rejected",
                        {"sink_id": self.config.id, "product_id": product.product_id, "quality": product.quality},
                    )
                    return
                ...
                # Generic product
                collected = CollectedProduct(
                    product_id="UNKNOWN", order_id=None, quality=1.0, collected_time=self.env.now
                )
    ...
    def _clean_throughput_window(self) -> None:
        ...
        self.recent_collections = [t for t in self.recent_collections if t > cutoff_time]
    ...
    def get_order_status(self, order_id: Optional[str] = None) -> Union[OrderTracking, Dict[str, OrderTracking], None]:
        ...
        if order_id:
            ...
            # Return all orders
            return {"active": self.active_orders, "completed": self.completed_orders}
