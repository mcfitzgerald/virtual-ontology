...
from typing import TYPE_CHECKING, Optional, Generator, Any, Union, List
...
if TYPE_CHECKING:
    from .scheduler import ProductionOrder
...
class SourcePrimitive(BasePrimitive):
    ...
    def set_production_order(self, order: "ProductionOrder") -> None:
        ...
        logger.info(
            f"{self.config.id}: Started order {order.order_id if order else 'None'}"
            + (" (changeover required)" if needs_changeover else "")
        )
    ...
    def _order_driven_generation(self) -> Generator:
        ...
        self.last_unit_generated = False  # Flag to track if unit was generated
        yield from self._generate_unit()

        # Only decrement if unit was actually generated (not rejected)
        if self.last_unit_generated:
        ...
        if self.arrival_rate > 0 and self.quality_rate > 0:
            self.arrival_rate * self.quality_rate
    ...
    def _execute_changeover(self) -> Generator:
        ...
        # Get changeover duration from configuration or scheduler
        changeover_duration = self.get_system_config("changeover.default_duration", 30.0)

        # Try to get actual changeover time from scheduler
        from .scheduler import SchedulerPrimitive
        ...
        if scheduler and isinstance(scheduler, SchedulerPrimitive):
            if self.last_product:
                ...
                if hasattr(self.env, "control_manager"):
                    ...
                    if isinstance(control_mgr, ControlManager):
                        ...
                        if smed_level == 1:
                            reduction = self.get_system_config("changeover.smed_level1_reduction", 0.5)
                            changeover_duration *= reduction
                        elif smed_level == 2:
                            reduction = self.get_system_config("changeover.smed_level2_reduction", 0.3)
                            changeover_duration *= reduction
        ...
        logger.info(
            f"{self.config.id}: Starting changeover from {self.last_product} to "
        ...
        setup_units = min(self.get_system_config("changeover.setup_units", 10), self.units_remaining)
        setup_scrap_rate = self.get_system_config("changeover.setup_scrap_rate", 0.15)
        ...
        for i in range(setup_units):
            if random.random() < setup_scrap_rate:
                ...
                # Generate good unit
                self.last_unit_generated = False
                yield from self._generate_unit()
                if self.last_unit_generated:
        ...
        if self.current_order:
            ...
            self.emit_observable(
                "order_completed",
                {
                    ...
                    "product_id": self.current_order.product.product_id
                    if hasattr(self.current_order, "product")
                    else "DEFAULT",
            ...
            logger.info(
                f"{self.config.id}: Completed order {self.current_order.order_id} "
    ...
    def _generate_unit(self) -> Generator:
        ...
        if random.random() > self.quality_rate:
            ...
            self.emit_observable("unit_rejected", {"source_id": self.config.id, "reason": "quality_check"})
            self.last_unit_generated = False
            yield self.env.timeout(0)  # Need to yield something for generator
            return
        ...
        unit = ProductionUnit(
            product_id=self.current_order.product.product_id
            if self.current_order and hasattr(self.current_order, "product")
            else "DEFAULT",
        ...
        if self.downstream:
            if hasattr(self.downstream, "put"):
                ...
                self.emit_observable(
                    "unit_generated",
                    {"source_id": self.config.id, "product_id": unit.product_id, "order_id": unit.order_id},
                )
            else:
    ...
    def _handle_disruption(self) -> Generator:
        ...
        if not self.is_disrupted:
            ...
            self.emit_observable("supply_disruption", {"source_id": self.config.id, "duration": disruption_duration})
    ...
    def get_statistics(self) -> dict:
        ...
        return {
            ...
            "quality_rate_actual": (
                ...
                if (self.total_generated + self.total_rejected) > 0
                else 0
            ),
    ...
    def initialize_wip(self, level: float = 0.5) -> Generator:
        ...
        if self.downstream and hasattr(self.downstream, "capacity"):
            ...
            for _ in range(initial_units):
                ...
                unit = ProductionUnit(product_id="INITIAL_WIP", quality=1.0, timestamp=0)

                if hasattr(self.downstream, "put"):
                    yield self.downstream.put(unit)

            self.emit_observable("wip_initialized", {"source_id": self.config.id, "units": initial_units})
