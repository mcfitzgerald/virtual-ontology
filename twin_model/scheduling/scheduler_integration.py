...
class SchedulerSimulationBridge:
            ...
            if self.product_manifest:
                ...
                if product:
                    ...
                    if product.constraints.optimal_batch_size > 0:
                        ...
                        target_volume = int(
                            product.constraints.optimal_batch_size * 
    ...
    def run_scheduled_production(self, duration_hours: float = 168) -> None:
        ...
        while self.env.now < end_time:
            ...
            for order_id, order in self.active_orders.items():
                if (order.status == OrderStatus.IN_PROGRESS and 
    ...
    def handle_disruption(
        ...
        """
        logger.warning(
            f"Disruption {disruption_type} affecting {affected_lines} "
            f"at time {self.env.now:.1f}"
        )
        ...
        for line_id in self.line_sources.keys():
            line_orders = [
                ...
                if o.line_id == line_id and o.status != OrderStatus.COMPLETED
            ]
    ...
    def get_schedule_metrics(self) -> Dict[str, Any]:
        ...
        if metrics['orders_scheduled'] > 0:
            metrics['completion_rate'] = (
                metrics['orders_completed'] / metrics['orders_scheduled'] * 100
            )
        
        # Calculate costs if available
        if self.cost_calculator and hasattr(self.scheduler, 'line_schedules'):
            schedule_cost = self.cost_calculator.calculate_schedule_cost(
                self.scheduler.line_schedules
            )
            metrics['total_cost'] = schedule_cost['total_cost']
            metrics['cost_per_order'] = (
                schedule_cost['total_cost'] / metrics['orders_scheduled']
                if metrics['orders_scheduled'] > 0 else 0
            )
    ...
    def export_schedule(self, filename: str = "schedule.csv") -> None:
        ...
        with open(filename, 'w', newline='') as csvfile:
            fieldnames = [
                'order_id', 'product_id', 'product_name', 'line_id',
                'scheduled_start', 'scheduled_duration', 'target_volume',
                'priority', 'status'
            ]
