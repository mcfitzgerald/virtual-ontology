...
from typing import Dict, List, Optional, Any, Generator, TYPE_CHECKING
import simpy
import logging
    ...
    def validate_schedule(
        ...
        if self.constraints.product_line_compatibility:
            for line_id, orders in schedule.items():
                for order in orders:
                    allowed_lines = self.constraints.product_line_compatibility.get(
                        order.product_id, []
                    )
                    if allowed_lines and line_id not in allowed_lines:
                        violations.append(
                            f"Product {order.product_id} not allowed on line {line_id}"
                        )
            ...
            for line_id, orders in schedule.items():
                for i in range(len(orders) - 1):
                    ...
                    if sequence in self.constraints.forbidden_sequences:
                        violations.append(
                            f"Forbidden sequence {sequence[0]} -> {sequence[1]} on line {line_id}"
                        )
                ...
                for i, order in enumerate(orders):
                    if order.product_id != current_product:
                        ...
                        if current_product and i > 0:
                            ...
                            if (self.constraints.min_campaign_length and 
                                campaign_hours < self.constraints.min_campaign_length):
                                violations.append(
                                    f"Campaign for {current_product} on line {line_id} "
                                    f"too short ({campaign_hours:.1f} hours)"
                                )
                            
                            if (self.constraints.max_campaign_length and 
                                campaign_hours > self.constraints.max_campaign_length):
                                violations.append(
                                    f"Campaign for {current_product} on line {line_id} "
        ...
        for line_id, orders in schedule.items():
            ...
            for order in orders:
                ...
                if current_product and current_product != order.product_id:
                    ...
                    changeover_time = self._estimate_changeover_time(
                        current_product, order.product_id
                    )
