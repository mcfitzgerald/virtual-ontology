...
from .production_scheduler import OrderStatus, ProductionOrder
...
class SubOptimalScheduleGenerator:
    ...
    def generate_schedule(
        self, duration_days: int = 14, start_date: Optional[datetime] = None
        ...
        while current_time < total_minutes:
            ...
            available_lines = [line for line, status in line_status.items() if status["until"] <= current_time]
            ...
            # Update line status
            line_status[line] = {"product": product, "until": current_time + batch_minutes}
    ...
    def _choose_product_suboptimally(self, line: str, previous_product: Optional[str]) -> Optional[str]:
        ...
        if self.config.sequence_mode == "random":
            ...
            if previous_product:
                ...
                different_group = [
                    p for p in capable_products if self.product_info[p]["changeover_group"] != prev_group
                ]
    ...
    def _determine_batch_size_suboptimally(self, product: str) -> float:
        ...
        if self.config.batch_sizing == "fixed":
            ...
            return random.uniform(self.config.min_batch_hours, self.config.max_batch_hours)

        elif self.config.batch_sizing == "random":
            # Random sizes with bias toward small
            base_size = random.uniform(self.config.min_batch_hours, self.config.max_batch_hours)
