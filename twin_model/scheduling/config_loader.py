...
def load_scheduling_constraints(config_path: Path) -> SchedulingConstraints:
    ...
    forbidden_sequences = [tuple(seq) for seq in constraints_data.get("forbidden_sequences", [])]
    preferred_sequences = [tuple(seq) for seq in constraints_data.get("preferred_sequences", [])]
...
def load_complete_scheduler_config(config_path: Path) -> Tuple[SchedulerConfig, SchedulingConstraints]:
...
def create_scheduler_from_config(
    env, config_path: Optional[Path] = None, catalog_path: Optional[Path] = None, random_seed: Optional[int] = None
):
    ...
    scheduler = ProductionScheduler(
        env=env, config=config, constraints=constraints, catalog_path=catalog_path, random_seed=random_seed
    )
...
def validate_schedule_config(config_path: Path) -> List[str]:
    ...
    if min_duration > max_duration:
        warnings.append(f"min_order_duration_hours ({min_duration}) > " f"max_order_duration_hours ({max_duration})")
        ...
        for line in lines:
            if line in line_products:
                if product not in line_products[line]:
                    warnings.append(
                        f"Inconsistency: {product} allowed on line {line} " f"but not in line_product_restrictions"
                    )
    ...
    if min_campaign > max_campaign:
        warnings.append(f"min_campaign_length_hours ({min_campaign}) > " f"max_campaign_length_hours ({max_campaign})")
