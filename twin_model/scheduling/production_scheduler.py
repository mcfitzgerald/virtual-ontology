...
class ProductionScheduler(BaseScheduler):
    ...
    def _default_product_catalog(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "SKU-1001": {"name": "8oz Water Bottle", "rate_per_5min": 475, "lines": ["1", "2"], "typical_batch": 50000},
            "SKU-1002": {"name": "8oz Juice", "rate_per_5min": 475, "lines": ["1", "2"], "typical_batch": 40000},
            "SKU-2001": {"name": "12oz Soda", "rate_per_5min": 475, "lines": ["2", "3"], "typical_batch": 60000},
            "SKU-2002": {
            ...
            "SKU-3002": {"name": "1L Juice", "rate_per_5min": 425, "lines": ["1", "2", "3"], "typical_batch": 30000},
        }
    ...
    def _load_product_catalog(self, path: Path) -> Dict[str, Dict[str, Any]]:
        ...
        for product_id, product_data in data.get("products", {}).items():
            catalog[product_id] = {
                ...
                "lines": [str(line) for line in product_data.get("allowed_lines", [])],
    ...
    def __init__(
        ...
        self.line_schedules: Dict[str, List[ProductionOrder]] = {"1": [], "2": [], "3": []}

        # Current production on each line
        self.current_production: Dict[str, Optional[ProductionOrder]] = {"1": None, "2": None, "3": None}
    ...
    def _setup_product_mixes(self) -> Dict[str, ProductMix]:
        ...
        return {
            ...
            "3": ProductMix(
                line_id="3", products=["SKU-2001", "SKU-2002", "SKU-3001", "SKU-3002"], weights=[0.35, 0.30, 0.20, 0.15]
            ),
        }
    ...
    def generate_order(
        self, line_id: str, scheduled_start: float, duration_hours: float = 4.0, product_id: Optional[str] = None
    ) -> ProductionOrder:
        ...
        if product_id is None:
            ...
            product_id = self.random.choices(product_mix.products, weights=product_mix.weights)[0]
        ...
        order = ProductionOrder(
            ...
            priority=self.random.randint(self.config.min_priority, self.config.max_priority),
        )
        ...
        for line_id in ["1", "2", "3"]:
            ...
            while current_time < simulation_duration:
                ...
                duration_hours = self.random.uniform(
                    self.config.min_order_duration_hours, self.config.max_order_duration_hours
                )

                # Create order
                order = self.generate_order(
                    line_id=line_id, scheduled_start=current_time, duration_hours=duration_hours
                )
                ...
                if len(self.line_schedules[line_id]) > 1:
                    ...
                    changeover_time = self.get_changeover_time(prev_order.product_id, order.product_id)
    ...
    def update_order_progress(self, order_id: str, good_units: float, scrap_units: float):
    ...
    def schedule_changeover(self, line_id: str) -> Generator:
        ...
        if current_order and next_order:
            changeover_time = self.get_changeover_time(current_order.product_id, next_order.product_id)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "completion_rate": (
                self.total_orders_completed / self.total_orders_created * 100 if self.total_orders_created > 0 else 0
            ),
            "orders_by_line": {},
            "orders_by_product": {},
        }
        ...
        for line_id in lines:
            ...
            while current_time < duration:
                ...
                if use_product_mix and line_id in self.product_mixes:
                    ...
                    valid_products = [
                        p for p in products if line_id in self.product_catalog.get(p, {}).get("lines", [])
                    ]
                ...
                order = self.generate_order(
                    line_id=line_id, scheduled_start=current_time, duration_hours=duration_hours, product_id=product_id
                )
        ...
        if objective == "minimize_changeover":
            ...
            for line_id, orders in current_schedule.items():
                ...
                sorted_products = sorted(
                    product_groups.items(), key=lambda x: sum(o.target_volume for o in x[1]), reverse=True
                )
                ...
                for product_id, product_orders in sorted_products:
                    ...
                    if product_id != sorted_products[-1][0]:
                        next_product = sorted_products[sorted_products.index((product_id, product_orders)) + 1][0]
            ...
            for line_id, orders in current_schedule.items():
                sorted_orders = sorted(
                    orders,
                    key=lambda o: self.product_catalog.get(o.product_id, {}).get("rate_per_5min", 0),
                    reverse=True,
                )
            ...
            for order in orders:
                if order.status == OrderStatus.COMPLETED:
                    ...
                    if order.scheduled_start <= disruption_time:
                        ...
                        completed_fraction = (disruption_time - order.scheduled_start) / order.scheduled_duration
                        remaining_duration = order.scheduled_duration * (1 - completed_fraction)
