...
class CampaignOptimizer(BaseScheduler):
    ...
    def generate_schedule(
        ...
        **kwargs
        ...
        for campaign in campaigns:
            ...
            best_line = self._find_best_line_for_campaign(campaign, lines, line_availability, last_product_on_line)
            ...
            if last_product and last_product != campaign.product_id:
                changeover_time = self._calculate_changeover_time(last_product, campaign.product_id)
        ...
        **kwargs
        ...
        if available_lines:
            return self.generate_schedule(
                orders=unfinished_orders, lines=available_lines, start_time=disruption_time + 60
            )
        else:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "average_changeover_minutes": (total_changeover_time / total_campaigns if total_campaigns > 0 else 0),
            "average_campaign_length_hours": (sum(campaign_lengths) / len(campaign_lengths) if campaign_lengths else 0),
        ...
        for order in orders:
            if order.product_id not in campaigns_dict:
                campaigns_dict[order.product_id] = Campaign(
                    product_id=order.product_id, orders=[], total_volume=0, total_duration=0
                )
