...
from .product_manifest import Product, ProductManifest
...
class ProductionCostCalculator:
    ...
    def __init__(
        ...
        if self.manifest.cost_factors:
            self.labor_cost_per_hour = self.manifest.cost_factors.get("labor_cost_per_hour", labor_cost_per_hour)
            self.energy_cost_per_kwh = self.manifest.cost_factors.get("energy_cost_per_kwh", energy_cost_per_kwh)
            self.overtime_multiplier = self.manifest.cost_factors.get("overtime_cost_multiplier", overtime_multiplier)
    ...
    def calculate_order_cost(
        ...
        # Production costs
        costs.production_cost = self._calculate_production_cost(product, order.target_volume)

        # Labor costs
        duration_hours = order.scheduled_duration / 60.0
        costs.labor_cost = self._calculate_labor_cost(duration_hours, line_id)
        ...
        if duration_hours > self.overtime_threshold_hours:
            ...
            costs.overtime_cost = overtime_hours * self.labor_cost_per_hour * (self.overtime_multiplier - 1.0)

        # Changeover costs
        if include_changeover and previous_product:
            costs.changeover_cost = self._calculate_changeover_cost(previous_product, order.product_id, line_id)

        # Waste costs
        costs.waste_cost = self._calculate_waste_cost(product, order.target_volume)

        # Quality costs
        costs.quality_cost = self._calculate_quality_cost(product, order.target_volume)

        # Inventory holding costs
        costs.inventory_cost = self._calculate_inventory_cost(product, order.target_volume, duration_hours)

        # Energy costs
        costs.energy_cost = self._calculate_energy_cost(product, order.target_volume, line_id)

        # Expedite costs if high priority
        if order.priority >= 8:
            expedite_multiplier = self.manifest.cost_factors.get("expedite_cost_multiplier", 1.5)
            costs.expedite_cost = costs.production_cost * (expedite_multiplier - 1.0)

        costs.calculate_total()
        return costs

    def _calculate_production_cost(self, product: Product, volume: float) -> float:
        ...
        Returns:
            Production cost
        """
        unit_cost = product.economics.material_cost + product.economics.packaging_cost + product.economics.overhead_cost
        return volume * unit_cost

    def _calculate_labor_cost(self, duration_hours: float, line_id: str) -> float:
    ...
    def _calculate_changeover_cost(self, from_product_id: str, to_product_id: str, line_id: str) -> float:
        ...
        changeover_time_hours = self.manifest.get_changeover_time(from_product_id, to_product_id) / 60.0
        labor_cost = changeover_time_hours * self.labor_cost_per_hour

        return base_cost + labor_cost

    def _calculate_waste_cost(self, product: Product, volume: float) -> float:
        ...
        disposal_cost_per_kg = self.manifest.cost_factors.get("waste_disposal_cost_per_kg", 0.05)
        disposal_cost = waste_weight_kg * disposal_cost_per_kg

        return waste_cost + disposal_cost

    def _calculate_quality_cost(self, product: Product, volume: float) -> float:
        ...
        """
        # Inspection costs
        inspection_cost_per_sample = self.manifest.cost_factors.get("quality_inspection_cost_per_sample", 2.0)
        samples = volume * product.quality.sampling_rate
        inspection_cost = samples * inspection_cost_per_sample

        # Expected defect costs
        defect_cost = 0.0
        for _defect_type, defect_info in product.quality.defect_categories.items():
            # Assume defect rate proportional to (1 - quality_rate)
            defect_rate = (1 - product.production.quality_rate) / len(product.quality.defect_categories)
            defect_units = volume * defect_rate
            defect_cost += defect_units * defect_info.get("cost", 0)

        return inspection_cost + defect_cost

    def _calculate_inventory_cost(self, product: Product, volume: float, duration_hours: float) -> float:
        ...
        holding_cost = average_inventory * product.economics.holding_cost_per_day * holding_days

        return holding_cost

    def _calculate_energy_cost(self, product: Product, volume: float, line_id: str) -> float:
    ...
    def calculate_schedule_cost(self, schedule: Dict[str, List[ProductionOrder]]) -> Dict[str, Any]:
        ...
        for line_id, orders in schedule.items():
            ...
            for order in orders:
                ...
                order_cost = self.calculate_order_cost(
                    order=order, line_id=line_id, include_changeover=True, previous_product=previous_product
                )
        ...
        total_volume = sum(order.target_volume for orders in schedule.values() for order in orders)

        total_time = sum(order.scheduled_duration for orders in schedule.values() for order in orders)
        ...
        return {
            ...
            "line_costs": {line_id: costs.to_dict() for line_id, costs in line_costs.items()},
            "product_costs": {product_id: costs.to_dict() for product_id, costs in product_costs.items()},
            "changeover_percentage": (
                total_costs.changeover_cost / total_costs.total_cost * 100 if total_costs.total_cost > 0 else 0
            ),
        }
    ...
    def compare_schedules(
        ...
        comparison["component_savings"] = {
            component: breakdown1[component] - breakdown2[component] for component in breakdown1
        }

        return comparison

    def find_cost_drivers(self, schedule: Dict[str, List[ProductionOrder]]) -> List[Dict[str, Any]]:
