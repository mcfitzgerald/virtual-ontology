...
class ParameterEffectsManager:
    ...
    def _define_parameters(self) -> None:
        ...
        self.parameters["micro_stop_probability"] = ParameterEffect(
            ...
            unit="probability",
            bounds=(0.05, 0.8),
            affects_primitives=["EquipmentPrimitive"],
        )
        ...
        self.parameters["changeover_duration"] = ParameterEffect(
            ...
            unit="minutes",
            bounds=(5.0, 120.0),
            affects_primitives=["EquipmentPrimitive", "SchedulerPrimitive"],
        )
        ...
        self.parameters["inventory_variance"] = ParameterEffect(
            ...
            unit="multiplier",
            bounds=(0.5, 3.0),
            affects_primitives=["SourcePrimitive", "SinkPrimitive"],
        )
    ...
    def get_parameters_for_primitive(self, primitive_type: str) -> Dict[str, ParameterEffect]:
        ...
        """
        return {name: param for name, param in self.parameters.items() if primitive_type in param.affects_primitives}
    ...
    def validate_parameter_set(self) -> List[str]:
        ...
        if (
            self.parameters["micro_stop_probability"].current_value > 0.5
            and self.parameters["performance_factor"].current_value > 1.0
        ):
            warnings.append("High micro-stop probability with high performance factor is unrealistic")

        if (
            self.parameters["scrap_rate"].current_value > 0.2
            and self.parameters["operator_skill_factor"].current_value > 1.2
        ):
            warnings.append("High scrap rate despite high operator skill is inconsistent")

        if (
            self.parameters["mtbf"].current_value < 60
            and self.parameters["maintenance_effectiveness"].current_value > 1.2
        ):
