...
class StructuredFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if record.exc_info:
            exception_data: Dict[str, Optional[Union[str, list[str]]]] = {
                "type": record.exc_info[0].__name__ if record.exc_info[0] else None,
                "message": str(record.exc_info[1]) if record.exc_info[1] else None,
                "traceback": traceback.format_exception(*record.exc_info) if self.json_format else None,
            }
            log_data["exception"] = exception_data  # type: ignore[assignment]
        ...
        if self.json_format:
            ...
            parts.append(str(log_data["message"]))
...
class SimulationLogger:
    ...
    def setup_logging(
        ...
        # Create formatters
        console_formatter = StructuredFormatter(json_format=False, include_context=True, include_timestamp=True)

        file_formatter = StructuredFormatter(json_format=json_format, include_context=True, include_timestamp=True)
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.debug(
                ...
                extra={"extra_data": {"function": func.__name__, "duration_ms": elapsed_ms, "status": "success"}},
            )
...
class EventInspector:
    ...
    def analyze_event_distribution(
        ...
        state_transitions = Counter((e.get("old_state"), e.get("new_state")) for e in state_changes)

        # Production analysis
        production_events = [e for e in events if e.get("event_type") in ["unit_produced", "unit_scrapped"]]

        # Equipment analysis
        equipment_events: Dict[str, list] = {}
        for event in events:
        ...
        analysis = {
            ...
            "events_per_equipment": {eq_id: len(eq_events) for eq_id, eq_events in equipment_events.items()},
        }

        self.logger.info("Event analysis complete", extra={"extra_data": analysis})
    ...
    def find_anomalies(self, events: list[Dict[str, Any]]) -> list[Dict[str, Any]]:
        ...
        for event in events:
            ...
            if issues:
                anomalies.append({"event": event, "issues": issues})
