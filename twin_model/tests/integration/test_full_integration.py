...
def load_manifests(manifest_dir: Path) -> dict:
    ...
    if equipment_path.exists():
        with open(equipment_path, "r") as f:
            ...
            logger.info(
                f"Loaded equipment manifest with {len(manifests['equipment_manifest'].get('equipment', {}))} equipment"
            )
    ...
    if production_path.exists():
        with open(production_path, "r") as f:
            ...
            logger.info(
                f"Loaded production manifest with {len(manifests['production_manifest'].get('products', {}))} products"
            )
...
def test_full_integration():
        ...
        for eq_entity in line.equipment:
            ...
            if eq_id in model["primitives"]:
                ...
                if hasattr(eq, "state_durations"):
                    ...
                    for state, duration_val in eq.state_durations.items():
                        if duration_val > 0:
                            ...
                            print(
                                f"      {state.value if hasattr(state, 'value') else state}: {duration_val:.1f} min ({pct:.1f}%)"
                            )
                ...
                if hasattr(eq, "state_durations"):
                    ...
                    running_time = eq.state_durations.get(EquipmentState.RUNNING, eq.state_durations.get("RUNNING", 0))
    ...
    for prim_id, prim in model["primitives"].items():
        if hasattr(prim, "micro_stop_probability"):
            print(
                f"  {prim_id}: micro_stop={prim.micro_stop_probability:.2f}, perf={prim.performance_factor:.2f}, scrap={prim.scrap_rate:.3f}"
            )
