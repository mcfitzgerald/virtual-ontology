...
def test_changeover_execution():
    ...
    for event in changeover_events:
        ...
        data = event['data']
        if 'duration' in data:
            logger.info(f"[{time:.1f}] Changeover started: {data.get('from_product', 'None')} -> "
            ...
            if 'setup_scrap' in data:
                logger.info(f"  Setup units: {data.get('setup_units', 0)}, "
                           f"Scrapped: {data.get('setup_scrap', 0)}")
...
def test_smed_impact():
    ...
    if results[0]['changeover_time'] > 0:
        for level in [1, 2]:
            time_reduction = (1 - results[level]['changeover_time'] / 
                            results[0]['changeover_time']) * 100
            productivity_gain = ((results[level]['productivity'] / 
                                results[0]['productivity']) - 1) * 100
