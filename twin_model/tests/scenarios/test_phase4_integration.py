...
from twin_model.primitives.scheduler import SchedulerPrimitive, ProductionOrder, Product, ProductCategory
...
class ProductionLine:
    ...
    def _create_scheduler(self, params: Dict[str, float]) -> SchedulerPrimitive:
        ...
        return SchedulerPrimitive(self.env, config)

    def _create_source(self, params: Dict[str, float]) -> SourcePrimitive:
        ...
        return SourcePrimitive(self.env, config)
    ...
    def _create_equipment_line(self, params: Dict[str, float]) -> list:
        ...
        for eq_id, eq_type, base_rate in equipment_specs:
            ...
            equipment_list.append(EquipmentPrimitive(self.env, config))

        return equipment_list

    def _create_sink(self, params: Dict[str, float]) -> SinkPrimitive:
        """Create sink."""
        config = PrimitiveConfig(
            id="SINK",
            type="SinkPrimitive",
        ...
        return SinkPrimitive(self.env, config)
...
def test_control_effects():
    ...
    assert kpis_high_speed["quality"] <= kpis_low_speed["quality"], "High speed should have worse quality"
...
def test_failure_modeling():
    ...
    sink = SinkPrimitive(env, PrimitiveConfig(id="SINK", type="SinkPrimitive", properties={}))
...
def test_scenario_progression():
    ...
    # Verify progression
    assert results["quick_wins"]["oee"] >= results["baseline"]["oee"], "Quick wins should improve over baseline"

    assert results["optimized"]["oee"] >= results["quick_wins"]["oee"], "Optimized should improve over quick wins"
