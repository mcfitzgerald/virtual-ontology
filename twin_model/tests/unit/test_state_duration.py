...
def test_state_duration_tracking():
    ...
    # Build model
    builder = OntologyDrivenModelBuilder(Path("ontology/twin_ontology.yaml"), Path("manifests"))
    ...
    if state_changes:
        ...
        for i, sc in enumerate(state_changes[:10]):
            ...
            print(
                f"  {i + 1}. @{sc['timestamp']:.1f}min: {sc['primitive']}: "
        ...
        if missing_duration == len(state_changes):
            ...
            print("  ⚠️  Some durations are 0 (expected for initial state changes)")
        else:
