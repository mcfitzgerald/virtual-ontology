...
class Buffer:
    ...
    capacity: int = 100
    current_level: int = 50
    min_operating_level: int = 10
    max_operating_level: int = 90
...
class LineCoupling:
    ...
    def __init__(self) -> None:
        ...
        line_config = self.config["line_coupling"]

        # Buffer parameters
        self.buffer_capacity = line_config["buffer"]["capacity"]
        self.initial_buffer_level = line_config["buffer"]["initial_level"]

        # Flow rates
        self.depletion_rate = line_config["flow_rates"]["depletion_rate"]
        self.refill_rate = line_config["flow_rates"]["refill_rate"]

        # Stochastic parameters
        self.depletion_noise_std = line_config["stochastic"]["depletion_noise_std"]
        self.refill_noise_std = line_config["stochastic"]["refill_noise_std"]
        self.use_probabilistic = line_config["stochastic"]["use_probabilistic"]

        # Cascade parameters
        self.cascade_sensitivity = self.config["parameters"]["cascade_sensitivity"][
            "default"
        ]
        self.cascade_delay_minutes = self.config["material_cascade"][
            "cascade_delay_base"
        ]
    ...
    def initialize_line(self, equipment_ids: List[str]) -> None:
        ...
        for i in range(len(equipment_ids) - 1):
            ...
            self.buffers[buffer_id] = Buffer(
                ...
                min_operating_level=self.config["line_coupling"]["buffer"][
                    "min_operating_level"
                ],
                max_operating_level=self.config["line_coupling"]["buffer"][
                    "max_operating_level"
                ],
            )
    ...
    def calculate_starvation(
        ...
        if time_interval_minutes is None:
            time_interval_minutes = self.config["sync_health"]["default_sync_interval"]
    ...
    def simulate_cascade(
        ...
        if time_steps is None:
            time_steps = self.config["line_coupling"]["simulation"]["time_steps"]
