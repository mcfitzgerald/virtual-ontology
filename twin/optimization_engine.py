...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("Example 1: Maximize OEE")
    print("="*60)
    
    objectives = [
        OptimizationObjective('oee', 'maximize', 1.0)
    ]
    ...
    if solutions:
        best = solutions[0]
        print(f"\nBest solution found:")
        print(f"  OEE: {best.objectives['oee']:.2f}%")
        print(f"  Parameters:")
    ...
    print("\n" + "="*60)
    print("Example 2: Multi-objective (OEE vs Energy)")
    print("="*60)
    
    objectives = [
        OptimizationObjective('oee', 'maximize', 1.0),
        OptimizationObjective('energy', 'minimize', 1.0)
    ]
    
    pareto_solutions = engine.optimize_multi_objective(
        objectives,
        population_size=30,
        generations=50,
        verbose=True
    ...
    for i, sol in enumerate(pareto_solutions[:3]):  # Show top 3
        print(f"\nSolution {i+1}:")
        print(f"  OEE: {sol.objectives.get('oee', 0):.2f}%")
        print(f"  Energy: {sol.objectives.get('energy', 0):.3f} kWh/unit")
    
    # Example 3: Scenario-based recommendation
    print("\n" + "="*60)
    print("Example 3: Natural Language Scenario")
    print("="*60)
    
    scenario = "optimize for energy efficiency while maintaining quality"
    recommendation = engine.recommend_for_scenario(scenario, verbose=False)
    
    print(f"\nScenario: {scenario}")
    print(f"Recommendation:")
    print(f"  Expected improvements:")
    for metric, value in recommendation.get('expected_improvements', {}).items():
