...
This guide provides working examples to get you started with the Virtual
Twin module quickly.
...
⚠️ **KPI Values**: All KPIs (OEE, availability, performance, quality)
are returned as **percentages (0-100)**, not fractions (0-1).

⚠️ **SimulationRun Access**: Use ``result.kpi_summary`` to access KPI
values, not ``result.kpis`` or ``result['kpi_summary']``.
...
**Problem**: The optimization may not be finding better solutions than
baseline.

**Solution**: 1. Check that you’re using a scenario that maps to
objectives 2. Try different scenarios or parameter ranges 3. Increase
population_size and generations for optimization
...
4. Explore the system prompt in ``SYSTEM_PROMPT.md`` for Claude Code
   integration
