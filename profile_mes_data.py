...
def analyze_production_metrics(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    prod_by_product = df.groupby('ProductID').agg({
        'GoodUnitsProduced': 'sum',
        'ScrapUnitsProduced': 'sum'
    }).sort_values('GoodUnitsProduced', ascending=False).head(10)
    ...
    for product, row in prod_by_product.iterrows():
        ...
        print(f"   {product:12s}: {row['GoodUnitsProduced']:8,} good, {row['ScrapUnitsProduced']:5,} scrap ({scrap_pct:4.1f}%)")
...
def generate_summary_report(df: pd.DataFrame, filepath: Path, 
                           quality: Dict, oee: Dict, production: Dict, 
                           downtime: Dict, temporal: Dict) -> None:
    ...
    print(f"  • Average OEE:        {oee['components']['availability']*oee['components']['performance']*oee['components']['quality']/10000:.1f}%")
...
def compare_datasets(df1: pd.DataFrame, df2: pd.DataFrame, 
                     name1: str = "Dataset 1", name2: str = "Dataset 2") -> None:
...
def main():
    ...
    parser.add_argument(
        "filepath",
        type=str,
        help="Path to MES CSV file to profile"
    )
    parser.add_argument(
        "--compare",
        type=str,
        help="Optional second file to compare against"
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    generate_summary_report(
        df, filepath, quality_analysis, oee_analysis, 
        production_analysis, downtime_analysis, temporal_analysis
    )
