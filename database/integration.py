...
from database.models import TwinRun, Experiment, DiscoveredPattern, ParameterRecommendation
...
class TwinDatabaseIntegration:
    ...
    def run_experiment(
        self, name: str, hypothesis: str, parameter_changes: Dict[str, Any], days: int = 1, num_runs: int = 3
    ) -> str:
        ...
        with Session(self.db_manager.engine) as session:
            ...
            if not baseline:
                ...
                baseline_id = self.run_simulation_to_db(run_type="baseline", days=days)
            ...
            experiment = exp_repo.create_experiment(
                ...
                baseline_run_id=baseline.run_id,  # type: ignore[union-attr]
            ...
            for i in range(num_runs):
                ...
                run_id = self.run_simulation_to_db(
                    ...
                    days=days,
                    parent_run_id=baseline.run_id,  # type: ignore[union-attr]
                )
            ...
            results_summary = self._analyze_experiment_results(
                session,
                baseline.run_id,  # type: ignore[union-attr]
                test_run_ids,
            )

            # Complete experiment
            conclusion = self._generate_conclusion(results_summary)
            exp_repo.complete_experiment(experiment.experiment_id, results_summary, conclusion)
    ...
    def _analyze_experiment_results(
        self, session: Session, baseline_id: str, test_run_ids: List[str]
    ) -> Dict[str, Any]:
        ...
        if comparisons:
            avg_oee_delta = sum(c["oee_delta"] for c in comparisons) / len(comparisons)  # type: ignore[misc]
            avg_production_delta = sum(c["production_delta"] for c in comparisons) / len(comparisons)  # type: ignore[misc]
        else:
            avg_oee_delta = 0
    ...
    def discover_patterns(self, min_confidence: float = 0.7) -> List[DiscoveredPattern]:
        """Discover patterns from completed experiments"""

        with Session(self.db_manager.engine) as session:
            # Query completed experiments
            statement = select(Experiment).where(Experiment.status == "completed")
            ...
            for exp in experiments:
                ...
                if results.get("avg_oee_delta", 0) > 5:
                    pattern = pattern_repo.create_pattern(
                        ...
                        pattern_name="Parameter set improves OEE",
    ...
    def generate_recommendations(self) -> List[ParameterRecommendation]:
        ...
        with Session(self.db_manager.engine) as session:
            ...
            statement = select(DiscoveredPattern).where(DiscoveredPattern.confidence_score >= 0.7)
