...
from database.repositories import PatternRepository, RecommendationRepository
...
async def create_experiment(request: ExperimentRequest, manager: ManagerDep) -> ExperimentResponse:
    """
    ...
    try:
        ...
        with Session(manager.engine) as session:
            ...
            if not experiment:
                raise HTTPException(status_code=500, detail="Experiment completed but record not found")
...
async def list_experiments(session: SessionDep, status: Optional[str] = None) -> ExperimentListResponse:
    """
    ...
    query = query.order_by(Experiment.created_at.desc())  # type: ignore[attr-defined]
    ...
    return ExperimentListResponse(experiments=responses, total=len(responses))


@router.get("/{experiment_id}", response_model=ExperimentResponse)
async def get_experiment(experiment_id: str, session: SessionDep) -> ExperimentResponse:
    """
...
async def list_patterns(
    session: SessionDep, min_confidence: float = 0.0, validated_only: bool = False
) -> PatternListResponse:
    """
    ...
    if validated_only:
        query = query.where(DiscoveredPattern.validated == True)  # noqa: E712

    query = query.order_by(DiscoveredPattern.confidence_score.desc())  # type: ignore[attr-defined]
    ...
    for pattern in patterns:
        ...
        responses.append(
            PatternResponse(
                pattern_id=pattern.pattern_id or 0,
...
async def discover_patterns(manager: ManagerDep, min_confidence: float = 0.7) -> PatternListResponse:
    """
...
async def validate_pattern(pattern_id: int, validation_run_ids: List[str], session: SessionDep) -> Dict[str, str]:
    """
    ...
    return {"message": f"Pattern {pattern_id} validated with {len(validation_run_ids)} runs"}
...
async def list_recommendations(
    session: SessionDep, applied_only: bool = False, min_confidence: float = 0.0
) -> RecommendationListResponse:
    """
    ...
    if applied_only:
        query = query.where(ParameterRecommendation.applied == True)  # noqa: E712
    if min_confidence > 0:
        query = query.where(ParameterRecommendation.confidence >= min_confidence)

    query = query.order_by(ParameterRecommendation.confidence.desc())  # type: ignore[attr-defined]
    ...
    for rec in recommendations:
        ...
        responses.append(
            RecommendationResponse(
                recommendation_id=rec.recommendation_id or 0,
    ...
    return RecommendationListResponse(recommendations=responses, total=len(responses))


@router.post("/recommendations/generate")
async def generate_recommendations(manager: ManagerDep) -> RecommendationListResponse:
    """
...
async def apply_recommendation(recommendation_id: int, session: SessionDep, manager: ManagerDep) -> SimulationResponse:
    """
    ...
    if recommendation.applied:
        raise HTTPException(status_code=400, detail="Recommendation has already been applied")
    ...
    actual_improvement: Dict[str, float] = {"oee": 0.0, "production": 0.0}  # Placeholder

    # Mark recommendation as applied
    rec_repo = RecommendationRepository(session)
    rec_repo.apply_recommendation(recommendation_id, run_id, actual_improvement)

    # Get run details for response
    with Session(manager.engine) as session:
        run = session.get(TwinRun, run_id)
        if not run:
            raise HTTPException(status_code=500, detail="Run created but not found in database")
        kpi_summary = None
