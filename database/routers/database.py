...
from database.schemas import (
    DatabaseStats, CleanupRequest, BackupRequest, BackupResponse,
    OperationResponse, TableInfo
)
from database.models import (
    HistoricalMESData, TwinRun, SimulationData,
    EquipmentConfig, ProductConfig, Experiment,
    DiscoveredPattern, ParameterRecommendation
)
...
async def database_status(
    ...
    try:
        ...
        # Get table counts
        table_counts = {
            "historical_mes_data": session.exec(select(func.count(HistoricalMESData.id))).one(),
            "twin_runs": session.exec(select(func.count(TwinRun.run_id))).one(),
            "simulation_data": session.exec(select(func.count(SimulationData.id))).one(),
            "equipment_config": session.exec(select(func.count(EquipmentConfig.equipment_id))).one(),
            "product_config": session.exec(select(func.count(ProductConfig.product_id))).one(),
            "experiments": session.exec(select(func.count(Experiment.experiment_id))).one(),
            "patterns": session.exec(select(func.count(DiscoveredPattern.pattern_id))).one(),
        }
...
async def database_statistics(
    ...
    tables = {
        ...
        "product_config": session.exec(select(func.count(ProductConfig.product_id))).one(),
        "experiments": session.exec(select(func.count(Experiment.experiment_id))).one(),
        "discovered_patterns": session.exec(select(func.count(DiscoveredPattern.pattern_id))).one(),
        "parameter_recommendations": session.exec(select(func.count(ParameterRecommendation.recommendation_id))).one(),
    }
...
async def import_historical_data(
    ...
    try:
        ...
        return OperationResponse(
            success=True,
            message=f"Historical data imported from {csv_file.filename}"
        )
...
async def create_backup(
    ...
    try:
        ...
        with Session(manager.engine) as session:
            total_records = (
                session.exec(select(func.count(HistoricalMESData.id))).one() +
                session.exec(select(func.count(SimulationData.id))).one() +
                session.exec(select(func.count(TwinRun.run_id))).one()
            )
...
async def restore_backup(
    ...
    try:
        ...
        if backup_file.filename.endswith(".gz"):
            import gzip
...
async def clean_database(
    ...
    try:
        ...
        if request.category:
            ...
            return OperationResponse(
                success=False,
                message="Must specify either category or tables to clean"
            )
        ...
        for table_name in tables_to_clean:
            if table_name == "historical_mes_data":
                ...
                records = session.exec(select(SimulationData)).all()
                for record in records:
                    session.delete(record)
                records_removed += len(records)
                
            elif table_name == "twin_runs":
                records = session.exec(select(TwinRun)).all()
                for record in records:
                    session.delete(record)
                records_removed += len(records)
    ...
    try:
        ...
        return OperationResponse(
            ...
            details={
                ...
                "configurations_preserved": keep_configurations
            }
        )
