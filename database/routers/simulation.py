...
from database.repositories import SimulationDataRepository, KPIRepository
...
async def run_simulation(request: SimulationRequest, manager: ManagerDep) -> SimulationResponse:
    """
    ...
    try:
        ...
        with Session(manager.engine) as session:
            ...
            if not run:
                raise HTTPException(status_code=500, detail="Simulation run completed but record not found")
...
async def list_simulations(
    ...
    query = query.order_by(TwinRun.started_at.desc())  # type: ignore[attr-defined]
    ...
    return SimulationListResponse(runs=run_responses, total=total, page=page, page_size=page_size)


@router.get("/{run_id}", response_model=SimulationResponse)
async def get_simulation(run_id: str, session: SessionDep) -> SimulationResponse:
    """
...
async def get_simulation_data(
    run_id: str, session: SessionDep, limit: int = Query(1000, ge=1, le=10000), offset: int = Query(0, ge=0)
) -> Dict[str, Any]:
    """
    ...
    # Get simulation data
    query = select(SimulationData).where(SimulationData.run_id == run_id).order_by(SimulationData.timestamp)  # type: ignore[arg-type]
    ...
    return {"run_id": run_id, "total_records": total, "offset": offset, "limit": limit, "data": data_dicts}


@router.get("/{run_id}/kpis", response_model=KPISummary)
async def get_simulation_kpis(run_id: str, session: SessionDep) -> KPISummary:
    """
    ...
    if not kpi_data:
        raise HTTPException(status_code=404, detail=f"No simulation data found for run '{run_id}'")

    return KPISummary(**kpi_data)


@router.post("/compare", response_model=ComparisonResponse)
async def compare_simulations(baseline_run_id: str, comparison_run_id: str, session: SessionDep) -> ComparisonResponse:
    """
    ...
    try:
        ...
        comparison = kpi_repo.create_comparison(baseline_run_id=baseline_run_id, comparison_run_id=comparison_run_id)
...
async def delete_simulation(run_id: str, session: SessionDep) -> Dict[str, str]:
    """
    ...
    sim_data = session.exec(select(SimulationData).where(SimulationData.run_id == run_id)).all()
    ...
    return {"message": f"Simulation run '{run_id}' and {len(sim_data)} data records deleted"}
