...
async def execute_query(request: SQLQueryRequest, session: SessionDep) -> SQLQueryResponse:
    """
    ...
    try:
        ...
        result = session.exec(text(query))  # type: ignore[call-overload]
...
async def list_tables(session: SessionDep, include_samples: bool = False) -> List[TableInfo]:
    """
    ...
    for table_name, model_class in ALLOWED_TABLES.items():
        try:
            ...
            # Get column names
            columns = [col.name for col in model_class.__table__.columns]  # type: ignore[attr-defined]

            # Get indexes
            indexes = [idx.name for idx in model_class.__table__.indexes]  # type: ignore[attr-defined]
            ...
            if include_samples and count > 0:
                ...
                sample_data = [{col: getattr(sample, col) for col in columns} for sample in samples]

            tables.append(
                TableInfo(
                    name=table_name, record_count=count, columns=columns, indexes=indexes, sample_data=sample_data
                )
            )
...
async def get_table_schema(table_name: str, session: SessionDep) -> Dict[str, Any]:
    """
    ...
    for fk in table.foreign_keys:
        schema["foreign_keys"].append(
            {"column": fk.parent.name, "references": f"{fk.column.table.name}.{fk.column.name}"}
        )
...
async def validate_query(request: SQLQueryRequest) -> Dict[str, Any]:
    ...
    return {"valid": True, "tables_referenced": tables_referenced, "message": "Query is valid and safe to execute"}
