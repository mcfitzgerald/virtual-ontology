...
def run_simulation(days, run_type):
    ...
    run_id = integration.run_simulation_to_db(run_type=run_type, days=days)
...
def run_experiment(name, hypothesis, days, runs):
    ...
    experiment_id = integration.run_experiment(
        name=name, hypothesis=hypothesis, parameter_changes=parameter_changes, days=days, num_runs=runs
    )
...
def status():
    ...
    with Session(manager.engine) as session:
        ...
        # Recent runs
        recent_runs = session.exec(select(TwinRun).order_by(TwinRun.started_at.desc()).limit(5)).all()
