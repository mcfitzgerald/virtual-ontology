...
from database.models import KPISnapshot
...
from twin_model.primitives.base import SamplingConfig, SimulationMode, ConsoleProgressReporter, SimulationMonitor
...
class OptimizedTwinDatabaseIntegration:
    ...
    def __init__(
        self, db_manager: Optional[TwinDatabaseManager] = None, perf_config: Optional[PerformanceConfig] = None
    ):
        ...
        with Session(self.db_manager.engine) as session:
            ...
            try:
                ...
                builder = OntologyDrivenModelBuilder(
                    Path(self.db_manager.ontology_path), Path(self.db_manager.manifest_dir)
                )
                ...
                # Setup event cache
                cache = ObservableCache(self.cache_dir, max_size=config.cache_max_size, dtype_size=512)
                ...
                if config.enable_monitoring:
                    ...
                    if memory_mb > config.max_memory_mb:
                        ...
                        print("  Switching to FAST mode for database operations")
                ...
                flush_stats = sim_repo.flush_cache_to_database(
                    run_id=run.run_id, cache_path=str(self.cache_dir), batch_size=config.batch_size
                )
                ...
                run_repo.update_run_status(run.run_id, "completed", kpi_summary)
    ...
    def run_performance_benchmark(self, days: float = 1, num_primitives: Optional[int] = None) -> Dict[str, Any]:
        ...
        configs_to_test = [ConfigPreset.FAST, ConfigPreset.PRODUCTION, ConfigPreset.MEMORY_OPTIMIZED]
        ...
        for preset in configs_to_test:
            ...
            try:
                ...
                with Session(self.db_manager.engine) as session:
                    ...
                    statement = select(KPISnapshot).where(KPISnapshot.run_id == run_id)
                ...
                results[preset.value] = {"error": str(e), "success": False}
    ...
    def auto_optimize_configuration(self, days: float, target_memory_mb: float = 500.0) -> PerformanceConfig:
        ...
        builder = OntologyDrivenModelBuilder(Path(self.db_manager.ontology_path), Path(self.db_manager.manifest_dir))
        ...
        config = PerformanceConfig().recommend_settings(
            num_primitives=num_primitives, simulation_days=int(days), available_memory_mb=target_memory_mb
        )

        # Estimate memory usage
        estimates = config.estimate_memory_usage(num_primitives, int(days))
