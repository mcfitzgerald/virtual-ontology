...
from twin_model.primitives.equipment_v2_fixed import EquipmentPrimitiveV2, EquipmentState
...
from twin_model.types import ProductionUnit
...
def create_test_line(env: simpy.Environment) -> Dict[str, Any]:
    ...
    for i, eq_id in enumerate(equipment_ids):
        ...
        equipment = EquipmentPrimitiveV2(
            env=env,
            config=eq_config,
            base_rate=80.0,
            performance_factor=0.85,
    ...
    sink_config = PrimitiveConfig(
        id="TEST-SINK",
        type="SINK",
        metadata={"name": "Test Sink"},
        properties={'collection_rate': 1000.0}
    )
    sink = SinkPrimitive(
        env=env,
        config=sink_config,
...
def test_material_flow():
    ...
    for eq_id in ['TEST-EQ1', 'TEST-EQ2', 'TEST-EQ3']:
        ...
        print(f"  OEE: {stats['oee']:.1%} (A:{stats['availability']:.1%} "
